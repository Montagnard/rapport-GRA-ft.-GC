...
#' luxury taxes. Rows: tax rates, then the maximum cost, the country bearing it, the RMSD and the new funds.
#' @param path Output .tex file.
#' @param caption,label Caption and label of the table.
#' @param ids Scenario ids shown for each model, in the order of the columns.
write_tex_merged <- function(path, caption, label, ids = c("A2", "A1", "B2", "C2")) {
  code_of <- setNames(names(country_codes), country_codes)
...
write_tex_merged("tables/table_min_heterogeneity_merged.tex",
                 sprintf("Tax rates minimising the heterogeneity of costs across Member States (total new revenues: EUR %.1f bn; rates capped at %s times the report's rates). Columns: minimising the sum of squared deviations of costs from the EU average (Sq.\\ dev.), minimising the maximum cost (Minimax), minimising squared deviations under the constraint that frugal countries pay less than the EU average (Frugal), and maximising the minimum State budget gain (Max.\\ gain).", target_bn, rate_cap),
