...
The concern of deterring investments due to lower returns does not apply here since the tax is one-off and backward-looking, so it does not alter the marginal return on any investment decided after 2027. % DONE: remove below

Another concern is that investment would be reduced since taxation would divert resources from productive investments into public consumption. This view negates the fact that public expenditures such as education, healthcare or research can often be described as investments rather than consumption, and even more profitable investments for society in the long run than short-term private investments. 
Moreover, if the revenue iq used to endow a European sovereign fund rather than spent (as proposed in Section~\ref{sec:fund}), assets withdrawn from private portfolios are re-invested, so the aggregate capital stock is preserved and ownership merely shifts from private to collective hands. On that route the fund's returns are designed to more than cover the servicing of the common debt, turning a one-off levy into a permanent public asset.
