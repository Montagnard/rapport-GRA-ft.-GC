...
The same implicit tax ratio is then applied to the equity trading volume of each Member State.\footnote{For some Member States, including Luxembourg, ESMA reports no equity trading volume in the dataset used for this exercise. These entries are therefore kept at zero for consistency. They should not be interpreted as implying the absence of financial-market activity, nor as meaning that these markets deal only in derivatives. They reflect the coverage and classification of the equity-trading data used here.} The estimate at a 0.3\% statutory rate is given by:
