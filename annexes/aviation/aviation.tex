...
%\author{Arved Dörpinghaus\thanks{Potsdam Institute for Climate Impact Research (PIK).}\and Matthias Kalkuhl, Lennart Stern, Adrien Fabre}
...
We present results for an efficiency-maximizing passenger-flow-preserving reform: it sets the vacant-seat tax at the level that delivers the efficient vacancy rate, then adjusts the additive VAT so that passenger numbers are unchanged. Because flows are fixed by construction, the reform places no burden on the tourism sector. Calibration gives a vacant-seat tax of 82.6\% of spoilage value and a 3.7\% additive VAT, cutting vacancy from 20.1\% to 2.7\% and flight frequency by 17.7\%. Unusually for a tax reform, it raises EUR~2.5 billion while generating surplus rather than burden: EUR~2.8 billion in partial equilibrium, plus EUR~1.8 billion once resources released from aviation flow to sectors carrying pre-existing taxes and markups, with a further EUR~0.3 billion in oil terms-of-trade gains and EUR~0.5 billion in avoided climate damages accruing to the EU.
...
\small
\begin{tabularx}{\textwidth}{lXXXXp{2.7cm}p{2.2cm}}
\toprule
 & & & & & \multicolumn{2}{c}{Revenue relative to GNI (bp)} \\
\cmidrule(lr){6-7}
 & Tax base & Share of & Share of UE& & \makecell[lb]{Passenger-flow-\\preserving Reform} & \makecell[lb]{Additional\\VAT of 20\%}\\
Member state & (EUR bn) & base (\%) & GNI (\%) & Ratio & (EUR 2.5bn) & (EUR 8bn) \\
\midrule
...
aggregate base is EUR~45 billion, with an estimation interval of EUR~38--52 billion.
